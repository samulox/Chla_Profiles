...
idir = os.path.abspath('/media/sf_Documents/DonnéesOLA_OBS2MOD/Premier approche')
figdir = opj(idir, 'figure')

fileS = opj(idir, 'Chla_sonde_essaii.csv')
fileCh = opj(idir, 'Chla_chimie_essaii.csv')

#fileAp = opj(idir, 'Approx_Chim_Sonde_Chl.csv')
fileAp = opj(idir, 'Approx_Chim_Sonde_Chl2.csv')

dfS = pd.read_csv(fileS, sep=';', index_col=1, parse_dates=True).dropna(axis=1)
dfS.columns = ['ID', 'profondeur', 'chl_a','par','Turbidité']

dfCh = pd.read_csv(fileCh, sep=';', index_col=1, parse_dates=True).dropna(axis=1)
dfCh.columns = ['ID', 'profondeur', 'chl_a']

dfAp = pd.read_csv(fileAp, sep=';', index_col=1, parse_dates=True).dropna(axis=1)
dfAp.columns = ['ID', 'profondeur', 'chl_a_S', 'chl_a_Ch']

df = dfAp
idir = os.path.abspath('/media/sf_Documents/Documents Tristan')
figdir = opj(idir, 'figure')

file = opj(idir, 'Chla_Chim_form.csv')

df = pd.read_csv(file, sep=';', index_col=4, parse_dates=True)#.dropna(axis=1)
df.columns = ['ID', 'nom du projet','site', 'platform', 'profondeur', 'depth_max', 'chl_a_Ch']
    #pd.read_csv(file, sep=';', index_col=3, parse_dates=True).dropna(axis=1)
#df.columns = ['ID', 'site', 'platform', 'profondeur', 'depth_max', 'chl_a']
#dg=df.profondeur<31
#df=df[dg]
...
B0_max = df[df.profondeur == 30].chl_a_Ch.max()
...
for idx, period in enumerate(([1, 3], [4, 6], [7, 9], [10, 12])):
    ...
    df[(df.month >= period[0]) & (df.month <= period[1])].plot.scatter('chl_a_Ch', 'profondeur', c='doy', cmap='Spectral',
                                                                       ax=axs[idx])
...
if optimization:
    ...
    for i,imonth in enumerate(imonths):
        ...
        for g_, df_ in data:
            ...
            # optimization
            z, chlCh= df_.profondeur, df_['chl_a_Ch'].values
            #chlS =df_['chl_a_S'].values
            resCh = solver(z, chlCh)
            #resS = solver(z, chlS)
            reslistCh.append([g_, *resCh.x])
            #reslistS.append([g_, *resS.x])
            # -------------

            color = cmap(float(ii) / N)
            #print("Valeur ii",ii)

            unique = tuple(list(dict.fromkeys(df.year)))
            #color = unique
            #color=['b', 'g', 'r', 'c', 'm', 'y', 'k','orange','teal','coral'] color[df_.year[0]-2003]
            c=osoaa_gauss_profile(depth, *resCh.x)
            print(g_)
            a=pd.DataFrame(c,depth)
            axs[i].plot(osoaa_gauss_profile(depth, *resCh.x), depth, '--', c=color,lw=2.5, alpha=0.6)
            axs[i].plot(df_['chl_a_Ch'], df_.profondeur, '-o', c=color, alpha=0.6, label=df_.year[0])
            print(osoaa_gauss_profile(depth, *resCh.x))
            #axs[i].plot(osoaa_gauss_profile(depth, *resS.x), depth, '--', c=color, lw=2.5, alpha=0.6)
            #axs[i].plot(df_['chl_a_S'], df_.profondeur, '-o', c=color, alpha=0.6, label=df_.year[0])
            ii += 1
            b=pd.concat([b,a])
    ...
    b.columns = ['chl_a']
    b.to_csv(opj(figdir, 'gaussian_fit_chl_leman_Chimie.csv'), index=True)

    resdfCh = pd.DataFrame.from_records(reslistCh)
    resdfCh.columns = ['date', 'B0', 'chl_max', 'z_max', 'sig']
    resdfCh['date'] = pd.to_datetime(resdfCh.date)
    resdfCh.to_csv(opj(figdir, 'gaussian_fit_chl_leman_Ch.csv'), index=False)

    #resdfS = pd.DataFrame.from_records(reslistS)
    #resdfS.columns = ['date', 'B0', 'chl_max', 'z_max', 'sig']
    #resdfS['date'] = pd.to_datetime(resdfS.date)
    #resdfS.to_csv(opj(figdir, 'gaussian_fit_chl_leman_S.csv'), index=False)
