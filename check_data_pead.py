...
# file = opj(idir, 'leman_chlorophylle_1430213014157-121850.csv')

# file = opj(idir, 'Chla_leman_chlorophylle_1614587779851-992590.csv')
# file = opj(idir, 'Chla_Chim_form.csv')

# file = opj(idir, 'Chla_Chim_form3.csv')
# file = opj(idir, 'Chla_leman_Sonde_form.csv')

# file = opj(idir, 'Chla_leman_Sonde_form30.csv')
# file = opj(idir, 'bourget__sonde.csv')
file = opj(idir, 'bourget_chlorophylle_chim.csv')

# file = opj(idir, 'Chl_a_moyen_mensuel_decennie_y.csv')

# file = opj(idir, 'Sonde_Leman_virgule.csv')


# df = pd.read_csv(file, sep=';', index_col=4, parse_dates=True)#.dropna(axis=1)
# df.columns = ['ID', 'nom du projet','site', 'platform', 'depth_min', 'depth_max', 'chl_a']

df = pd.read_csv(file, sep=',', index_col=4, parse_dates=True).dropna(axis=1)
df.columns = ['ID', 'nom du projet', 'site', 'platform', 'depth_obs_min', 'depth_obs_max', 'chl_a']

# df = pd.read_csv(file, sep=';', index_col=4, parse_dates=True)#.dropna(axis=1)
# df.columns= ['ID','nom du projet','site', 'platform', 'depth_min','chl_a']

# df = pd.read_csv(file, sep=';', index_col=1, parse_dates=True)#.dropna(axis=1)
# df.columns = ['ID', 'depth_min','chl_a']
# df.columns = ['ID', 'depth_min','chl_a','par','turbidite']
# dg=df.depth_min<31
# df=df[dg]
...
# set maximum depth from data set
depth_max = df.depth_obs_min.max()
depth = np.arange(0, depth_max, 0.1)
...
B0_max = df[df.depth_obs_min == depth_max].chl_a.max() * 1.2
...
for idx, period in enumerate(([1, 3], [4, 6], [7, 9], [10, 12])):
    ...
    df[(df.month >= period[0]) & (df.month <= period[1])].plot.scatter('chl_a', 'depth_obs_min', c='doy', cmap='Spectral',
                                                                       ax=axs[idx])
...
if optimization:
    ...
    for i, imonth in enumerate(imonths):
        ...
        for g_, df_ in data:
            ...
            # optimization
            z, chl = df_.depth_obs_min, df_['chl_a'].values
            res = solver(z, chl)
            ...
            # print("Valeur ii",ii)
            c = osoaa_gauss_profile(depth, *res.x)
            dte = np.array([])
            for q in range(np.shape(c)[0]):
                dte = np.concatenate([dte, np.array([g_.value])])
            ...
            axs[i].plot(df_['chl_a'], df_.depth_obs_min, '-o', c=color, alpha=0.6, label=df_.year[0])
            ii += 1
            b = pd.concat([b, a])
            e = pd.concat([e, d])
        axs[i].invert_yaxis()
    ...
    b.to_csv(opj(figdir, 'gaussian_fit_chl_leman_Sonde.csv'), index=True)

    e.columns = ['date']
    # e.to_csv(opj(figdir, 'gaussian_fit_chl_leman_Chimie_date.csv'), index=False)
    e.to_csv(opj(figdir, 'gaussian_fit_chl_leman_Sonde_date.csv'), index=False)
...
resdf['chl_tot'] = resdf.apply(
    lambda row: osoaa_gauss_integral(30, row['B0'], row['chl_max'], row['z_max'], row['sig']), axis=1)

import seaborn as sns

sns.set_style('white')
g = sns.pairplot(resdf.loc[:, (pardate, 'chl_tot', 'chl_max', 'z_max', 'sig')], hue=pardate, palette='Spectral',
                 aspect=1.5)
